...
def output(rewards, times, total_time, resolution='1x'):
    ...
    with open(f"{resolution}/{resolution}_experiment_results.txt", 'w') as f:
    ...
    plt.savefig(f"{resolution}/rewards{resolution}.jpg")
    plt.close()
    ...
    plt.savefig(f"{resolution}/time{resolution}.jpg")
    plt.close()
...
def stability(rewards):
    ...
    for i in range(1, len(rewards) - 1):
        delta1 = rewards[i] - rewards[i - 1]
        delta2 = rewards[i + 1] - rewards[i]
    ...
    # 1 - avg_error because we want stability/similarity in rate of change, not difference
...
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["Quarter"][0], label="0.25x")
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["Half"][0], label="0.5x")
...
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["100x"][0], label="100x")
plt.legend()
plt.xlabel('Episodes')
plt.ylabel('Average Reward')
plt.title('Average Reward vs Episodes')
...
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["Quarter"][1], label="0.25x")
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["Half"][1], label="0.5x")
...
plt.plot(100 * (np.arange(len(rewards)) + 1), outcomes["100x"][1], label="100x")
plt.legend()
plt.xlabel('Episodes')
plt.ylabel('Average Time (in seconds)')
plt.title('Average Time vs Episodes')
