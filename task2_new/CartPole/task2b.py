...
def QLearning(env, learning, discount, epsilon, min_eps, episodes, granularity):
    ...
    num_states = (env.observation_space.high -
                  env.observation_space.low) * np.array([10 * granularity, 100 * granularity])
    ...
    for i in range(episodes):
        ...
        while not done:
            ...
            state2_adj = (state2 - env.observation_space.low) * \
                         np.array([10 * granularity, 100 * granularity])
...
def output(rewards, times, total_time, granularity='1x'):
    ...
    plt.savefig(f"rewards{granularity}.jpg")
    plt.close()
    ...
    plt.savefig(f"time{granularity}.jpg")
    plt.close()
